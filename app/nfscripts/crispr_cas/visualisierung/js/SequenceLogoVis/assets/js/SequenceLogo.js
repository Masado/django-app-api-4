...
SequenceLogo.rendering = {
    ...
    createSequenceLogo: function (options) {
        ...
        SequenceLogo.rendering.processSequences(options.sequences);
    },

    processSequences: function (sequences) {
        SequenceLogo.variables.fileCount = sequences.length;
        SequenceLogo.variables.plotHeight = SequenceLogo.variables.height / sequences.length;

        for (var fileIndex in sequences) {
            var sequenceData = d3.csv.parse(sequences[fileIndex]);
			console.log("SequenceData parsed from CSV text:");
