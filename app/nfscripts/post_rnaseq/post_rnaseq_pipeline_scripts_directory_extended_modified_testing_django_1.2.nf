...
process KeyPathwayMiner{
	...
	path 'KPM_genes.txt' into kpmGenes_ch
	val 3 into tex_wait_ch
	val 4 into copy_wait_ch2
	
	"""
	Rscript $script/KeyPathwayMiner.r --matrix $matrix --network $network --pathways $cpn --Kmin $kmin --Kmax $kmax --Kstep $kstep --Lmin $lmin --Lmax $lmax --Lstep $lstep --out $o/images --out_genes "KPM_genes.txt"
...
process gprofiler {
	...
	input:
	val org from org_ch3
	val script from scripts_ch4
	path genes from kpmGenes_ch
	val o from out_ch5
	...
	"""
	Rscript $script/gprofiler_local.R --query $genes --organism "$org" --out .
	"""
}
