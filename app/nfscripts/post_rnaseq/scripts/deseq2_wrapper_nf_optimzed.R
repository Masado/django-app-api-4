...
#samples <- read.table("samples_extended.txt")
...
#compare <- data.table(read.table("chicken_compare.tsv"))
for (x in compare) {
	file_name <- paste(x[1], "_vs_", x[2], ".csv", sep="")
	write.csv(as.data.frame(results(dds, contrast = c("condition", x[1], x[2]))[order(results(dds, contrast = c("condition", x[1], x[2]))$log2FoldChange),]), file=paste(getwd(), "/", file_name, sep=""))
}
