...
if (params.bed)  { 
	...
	process MAKE_BED {
		...
		output:
		path '*.bed' into extract_genes_bed, matrix_creation_bed, collected_matrix_bed
...
process chromosome_extraction {
	tag "${ext_chr}"
	publishDir "${params.outdir}/genome/${ext_chr}/"
...
process MATRIX_GENERATION {
	tag "$name"
	publishDir "${params.outdir}/matrix/${name}/"
...
process MATRIX_GENERATION_COLLECTED {
	tag "collected"
	publishDir "${params.outdir}/matrix/collected/"
...
process MATRIX_GENERATION_CHROMOSOMES {
	tag "$name"
	publishDir "${params.outdir}/matrix/${ext_chr}/${name}/"
...
process MATRIX_GENERATION_CHROMOSOMES_COLLECTED {
	...
	publishDir "${params.outdir}/matrix/chr_collected/"
...
process HEATMAP_PLOTTING{
	tag "$name"
	publishDir "${params.outdir}/visualization/${name}/"
...
process HEATMAP_PLOTTING_COLLECTED {
	tag "collected"
	publishDir "${params.outdir}/visualization/collected/"
...
process HEATMAP_PLOTTING_CHROMOSOMES {
	tag "$name"
	publishDir "${params.outdir}/visualization/${ext_chr}/"
...
process HEATMAP_PLOTTING_CHROMOSOMES_COLLECTED {
	tag "chromosomes_collected"
	publishDir "${params.outdir}/visualization/chr_collected/"
